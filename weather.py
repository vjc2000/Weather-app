...
st.set_page_config(page_title='Vaibhav Chaudhari', page_icon=":rainbow:")
...
if(st.button("SUBMIT")):
    try:
        ...
        line_color='black',fill_color=['paleturquoise',['palegreen', '#fdbe72']*7], font_size=14,height=32,font=dict(color='black', size=14)
            ))])
        ...
        line_color='black',fill_color=['paleturquoise',['palegreen', '#fdbe72']*7], font_size=14,height=36, font=dict(color='black', size=14)))])
        ...
        st.header(' ')
        st.header(' ')
        st.markdown("Made with love by Vaibhav Chaudhari")
    except KeyError:
